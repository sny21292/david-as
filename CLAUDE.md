...
**Gems & Gemology library** (`gems-gemology.html` + `js/bookshelf.js` + `css/bookshelf.css`): article data lives in `data/articles.json`; the JS renders each article as a clickable book spine (rows of 13, built above the static decor shelves anchored at `#decorShelves`). Full articles render on `article.html` at clean URLs `/articles/<slug>` — a single global `.htaccess` rule rewrites those to `article.html?id=<slug>`, and the page also parses the slug from the path, so **never add per-article rewrite rules**; new articles only need a JSON entry. `article.html` must use absolute (`/`-prefixed) asset and fetch paths because it serves from a subpath. Clean article URLs don't work under `php -S` (no .htaccess) — test locally with `article.html?id=<slug>`. The nav's "Gems & Gemology" item replaced the old "Gem of Month" link; `gem.html` still exists at `/gem` but is no longer linked.
...
- Hostinger serves CSS/JS with `max-age=604800` (7 days), so every stylesheet/script reference in HTML carries a cache-bust query (`css/style.css?v=YYYYMMDD`). **When you edit any CSS or JS file, bump the `?v=` value in every HTML page that references it** — otherwise returning visitors run stale code for up to a week.
